...
//        this.userList.add(new UserEntity("nick3","login3", "passw3"));
    ...
    public void start() {
        ...
        ServerImpl.logger.info("Сервис аутентификации запущен");
    }
    ...
    public UserEntity getUser(String login, String password) {
        ...
        if (user != null) {
            if ((user.getName().equals(login)) & (user.getPassword().equals(password))) {
                ServerImpl.logger.info("Попытка авторизации пользователя " + user.getName() + " успешна.");
                return user;
            } else {
                ServerImpl.logger.info("Попытка авторизации пользователя " + user.getName() + " не успешна.");
            }
        }
        return null;
    }
